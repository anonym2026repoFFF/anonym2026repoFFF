...
class oldFFF(nn.Module):
	...
	def training_forward(self, x_orig: torch.Tensor, return_entropies: bool=False, use_hard_decisions: bool=False):
		"""
		...
		x = x_orig.view(-1, x_orig.shape[-1])
		...
		final_logits_ret = final_logits.view(*original_shape[:-1], self.output_width)	# (..., self.output_width)

		if not return_entropies:
			return final_logits_ret
		else:
			return final_logits_ret, entropies.mean(dim=0)
