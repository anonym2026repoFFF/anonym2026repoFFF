...
class FastOPT(OPTForCausalLM):
    ...
    config_class = FastOPTConfig

    def __init__(self, config, depth=3, strategy="new_FFF", moe_n_experts=4, moe_k=2):
        super().__init__(config)

        self.depth = getattr(config, "depth", 3)
        self.strategy = getattr(config, "strategy", "new_FFF")
        self.moe_n_experts = getattr(config, "moe_n_experts", 4)
        self.moe_k = getattr(config, "moe_k", 2)
