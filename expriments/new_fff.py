...
class FFF(nn.Module):
	...
	def forward(self, oldx: torch.Tensor) -> torch.Tensor:
		...
		with torch.no_grad():
			# this is a pointer to the currently active node idx in the hierarchy, we start at root
			...
			for d in range(self.depth):
				...
				# get decision for each of the parallel trees for the current level
				moves = torch.gather(decisions, 2, current_nodes.unsqueeze(2)).squeeze(2)
				# calculate indices of next nodes
